...
export default function MultipleRecordLinksCell({ cell, rowData }) {
    ...
    function addSelectedRow(ele) {
        ...
        setSelectedRowData([...selectedRowData, { data: ele.data, recordId: ele.id }])
    ...
    function removeSelectedRow(ele) {
        ...
        let selectedRowRecordId = updatedSelectedRowData.map(({ recordId }) => recordId)
                    ...
                    selectedRowData?.map((ele, i) => {
                        return (
                            <div key={i} className='flex bg-purple-100 rounded-md items-center cursor-pointer px-1'>
                                <div className='h-full w-max'>{ele.data[cell.column.columnDef.linked_rec.selected_field_name]}</div>
                            ...
                            >
                                <Dialog.Panel className="w-[600px] h-[600px]  transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                                    ...
                                    <div className="h-[500px] overflow-y-auto">
                                        {
                                            isDataFetched ? (
                                                responseGetTableData?.data?.filter((element) => {
                                                    ...
                                                    return data[primaryData].toLowerCase().trim().includes(searchTerm.toLowerCase().trim())
                                                }).map((ele, i) => {
                                                    return <div onClick={() => {
                                                        closeModal()
                                                        addSelectedRow(ele)
                                                        ...
                                                        <div className='font-medium truncate'>{ele.data[primaryData] || "empty field"}</div>
                                                        <div></div>
                                                    </div>
                                                })
                                            ) : <Loading />
